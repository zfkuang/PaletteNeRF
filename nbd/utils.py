...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        if self.opt.hybrid:
            ...
            sparsity_loss = (2-pred_omega).mean()
            pred_radiance = outputs['radiance']
            dir_loss = pred_radiance[:,self.opt.sg_num_basis:].mean()
            loss = loss + self.opt.lambda_sparsity * sparsity_loss + self.opt.lambda_dir * dir_loss
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                for i in range(self.opt.num_basis):
                    ...
                    basis_roughness = self.model.basis_roughness[i,None,None].repeat(100, 100)
                    pred_basis_roughness.append(basis_roughness.detach().cpu().numpy())
                ...
                pred_basis_roughness = (np.concatenate(pred_basis_roughness, axis=1).clip(0,1)*255).astype(np.uint8)
                ...
                if write_video:
                    ...
                    cv2.imwrite(os.path.join(save_path, f'{name}_{i:04d}_basis_roughnes.png'), pred_basis_roughness)
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                if self.local_rank == 0:
                    ...
                    save_path_basis_roughness = os.path.join(self.workspace, 'validation', f'{name}_{self.local_step:04d}_basis_roughness.png')
                    ...
                    cv2.imwrite(save_path_basis_roughness, pred_basis_roughness)
