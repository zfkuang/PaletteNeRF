...
class LSeg_MultiEvalModule(DataParallel):
    ...
    def forward(self, image, label_set=''):
        ...
        with torch.cuda.device_of(image):
            ...
            feat_maps = image.new().resize_(batch,512,h,w).zero_().cuda()
        ...
        for scale in self.scales:
            ...
            if long_size <= crop_size:
                ...
                for idh in range(h_grids):
                    for idw in range(w_grids):
                        ...
                        h1 = min(idh * stride + crop_size, ph)
                        w1 = min(idw * stride + crop_size, pw)
                        h0 = max(0, h1-crop_size)
                        w0 = max(0, w1-crop_size)
                        ...
                        output = resize_image(output, h1-h0, w1-w0, **self.module._up_kwargs)
            ...
            feat_map = resize_image(outputs, h, w, **self.module._up_kwargs)
            feat_maps += feat_map
        feat_maps /= len(self.scales)
        return feat_maps
