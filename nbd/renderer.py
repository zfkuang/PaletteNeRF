...
class NBDRenderer(nn.Module):
    def __init__(self,
        ...
        self.freeze_basis_color = opt.use_initialization_from_rays
        self.color_weight = 0
        self.opt = opt
        ...
        if self.opt.roughness_list[0] >= 0:
            num_basis = self.opt.sg_num_basis if self.opt.hybrid else self.num_basis
            assert(num_basis%len(self.opt.roughness_list) == 0)
            stride = num_basis // len(self.opt.roughness_list)
            for i, roughness in enumerate(self.opt.roughness_list):
                self.basis_roughness[i*stride:(i+1)*stride] = roughness
        self.basis_roughness = nn.Parameter(self.basis_roughness, requires_grad=opt.optimize_roughness)
    ...
    def run(self, rays_o, rays_d, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        mu, omega, roughness, radiance, color = self.color(xyzs.reshape(-1, 3), dirs.reshape(-1, 3), 
                                                    mask=mask.reshape(-1), **density_outputs)
        mu = mu.reshape(N, -1, self.num_basis, 3)
        omega = omega.reshape(N, -1, self.num_basis, 1)
        roughness = roughness.reshape(N, -1, self.num_basis, 1)
        radiance = radiance.reshape(N, -1, self.num_basis, 1)
        ...
        basis_roughness = self.basis_roughness[None,None,:,None].clamp(0.05, 1)
        if self.opt.add_point_roughness:
            roughness = basis_roughness+roughness
        else:
            roughness = basis_roughness


        if self.opt.hybrid:
            if self.opt.no_omega:
                omega = torch.ones_like(omega)

            sg_num_basis = self.opt.sg_num_basis
            dir_num_basis = self.opt.num_basis - self.opt.sg_num_basis
            
            if self.opt.no_sg:
                basis_rgb_sg = (omega*radiance*basis_color)[:,:,:self.opt.sg_num_basis,:]
            else:
                sharpness = 2/roughness/roughness-1.99 # Approximation from NDF to SG
                integral = 2*torch.pi / sharpness * (1-torch.exp(-2*sharpness)) 
                integral = integral/10 # for faster convergence
                amp = omega*radiance*basis_color / integral
                def calc_SG(mu, sharpness, amp, x):
                    return amp * torch.exp(sharpness*((x*mu).sum(dim=-1, keepdim=True)-1))
                basis_rgb_sg = calc_SG(mu, sharpness, amp, rays_d[:,None,None,:])[:,:,:self.opt.sg_num_basis] # (N_rays, N_samples_, N_basis, 3)
                
            basis_rgb_dir = (omega*radiance*basis_color)[:,:,self.opt.sg_num_basis:,:]
            basis_rgb = torch.cat([basis_rgb_sg, basis_rgb_dir], dim=-2)

            if self.opt.no_omega:
                radiance_sg_norm = radiance[...,:self.opt.sg_num_basis, 0] / (1e-6+radiance[...,:self.opt.sg_num_basis,0].sum(dim=-1, keepdim=True)).detach() # N_rays, N_sample, N_B
                radiance_dir_norm = radiance[...,self.opt.sg_num_basis:, 0] / (1e-6+radiance[...,self.opt.sg_num_basis:,0].sum(dim=-1, keepdim=True)).detach() # N_rays, N_sample, N_B
                radiance_norm = torch.cat([radiance_sg_norm, radiance_dir_norm], dim=-1)
                radiance_norm = (radiance_norm**2).sum(dim=-1, keepdim=True) # N_rays, N_sample, 1
                omega_norm_map = (weights[:,:,None].detach()*radiance_norm).sum(dim=1) # N_rays, N_B
                omega_norm_map = omega_norm_map.view(*prefix, 1)
            else: 
                rgb_sg_norm = basis_rgb_sg.sum(dim=-1)
                rgb_sg_norm = rgb_sg_norm / (1e-6+rgb_sg_norm.sum(dim=-1, keepdim=True)).detach() # N_rays, N_sample, N_B
                rgb_sg_norm = rgb_sg_norm.norm(dim=-1, p=float('inf'), keepdim=True)
                
                rgb_dir_norm = basis_rgb_dir.sum(dim=-1)
                rgb_dir_norm = rgb_dir_norm / (1e-6+rgb_dir_norm.sum(dim=-1, keepdim=True)).detach() # N_rays, N_sample, N_B
                rgb_dir_norm = rgb_dir_norm.norm(dim=-1, p=float('inf'), keepdim=True)
                
                omega_norm = rgb_sg_norm+rgb_dir_norm
                # omega_norm = (omega[...,0]**2).sum(dim=-1, keepdim=True) # N_rays, N_sample, 1
                omega_norm_map = (weights[:,:,None].detach()*omega_norm).sum(dim=1) # N_rays, N_B1
                omega_norm_map = omega_norm_map.view(*prefix, 1)
            radiance_map = (weights[:,:,None].detach()*radiance[...,0]).sum(dim=1) # N_rays, N_B
            radiance_map = radiance_map.view(*prefix, self.num_basis)
        elif self.opt.no_sg:
            basis_rgb = omega*radiance*basis_color
        else:
            sharpness = 2/roughness/roughness-1.99 # Approximation from NDF to SG
            integral = 2*torch.pi / sharpness * (1-torch.exp(-2*sharpness)) 
            integral = integral/10 # for faster convergence
            ...
            def calc_SG(mu, sharpness, amp, x):
                return amp * torch.exp(sharpness*((x*mu).sum(dim=-1, keepdim=True)-1))
            basis_rgb = calc_SG(mu, sharpness, amp, rays_d[:,None,None,:]) # (N_rays, N_samples_, N_basis, 3)
            
        rgb = basis_rgb.sum(dim=-2) # (N_rays, N_samples_, 3)
        if self.opt.guidance:
            rgb = rgb*(1-self.color_weight) + color*self.color_weight
        rgb_map = (weights[:,:,None]*rgb).sum(dim=1) # N_rays, 3
        rgb_map = safe_pow(rgb_map, 1/2.4)
        # rgb_map = safe_linear_to_srgb(rgb_map)
        ...
        # basis_rgb_map = safe_linear_to_srgb(basis_rgb_map)
        basis_acc_map = (weights[:,:,None].detach()*omega[...,0]*radiance[...,0]).sum(dim=1)
